...
def colMapToDistField(col_map):
        ...
        traj1_offset2 = traj1 + np.array(len(traj1)*[[-len(col_map),0]])
        traj1_offset3 = traj1 + np.array(len(traj1)*[[len(col_map),0]])
                        ...
                        for pt in traj:   
                                ...
                                if d < curr_point_dist:
                                        ...
                                        if col_map[i,j] > 0:
                                                ...
                                                sign = -1
                        dist_field_j.append(sign*curr_point_dist)
                dist_field.append(dist_field_j)
        return np.sqrt(dist_field)
