...
def buildData(distance2D):
    ...
    for i in range(n):
        for j in range(m):
            X.append([np.cos(np.pi*i/n),np.cos(np.pi*j/m)])
            #X.append([i/n,j/m])
...
def buildPred(reg, distance2D):
    ...
    for i in range(n):
        pred_i = []
        for j in range(m):
            pred_i.append(reg.predict(np.array([np.cos(np.pi*i/n),np.cos(np.pi*j/m)]).reshape(1,-1)))
...
reg = MLPRegressor(hidden_layer_sizes=(8,6,4),verbose=True, max_iter=400, activation='relu')
...
wrong_pred, lost_space, pred_error = computePredictionError(pred, dist_field, 0, optim=False, optimRate=1)
